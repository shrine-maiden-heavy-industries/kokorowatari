...
Hey! Thanks for your interest in contributing to Kokorowatari!

A few important things to note up front:

* Contributions to the Kokorowatari source code are licensed under the [BSD-3-Clause], the full text of which can be found in the [`LICENSE`] file in the root of the [git repository].
* Contributions to the Kokorowatari documentation are licensed under the [CC-BY-SA 4.0], the full text of which can be found in the [`LICENSE.docs`] file in the root of the [git repository].
* When contributing to Kokorowatari and interacting with people in/around the project be sure to keep in mind the [Code Of Conduct].
...
The goal for Kokorowatari is to be able to be as friendly and powerful as possible, allowing for anyone of any skill level to get into hardware design, as such it is extremely important that we get feedback from users so we know what snags, sharp corners, unclear information, and unexpected behavior they encounter.

It is asked that you please report any and all problems you encounter when using Kokorowatari, this can be from small things such as typos or weird wording, and unclear/confusing messages and diagnostics, to larger things such as general useability and tool integration.

Reporting an issue with Kokorowatari is done through the [issue tracker], the issue templates should guide you through most of the needed information. A quick summary of what would be needed is as follows:

* The version of Kokorowatari being used, this can be done by running `python -c "import kokorowatari;print(kokorowatari.__version__)"`
...
We are trying to make Kokorowatari as widely useable and capable as possible, as such we love hearing about things that would help us achieve that goal.
...
The guidelines and instructions below are a best-effort attempt at describing the least-friction way of getting started with doing development and testing for Kokorowatari. If you notice any problems then please do [report an issue], and if you need any help then feel free to use the [discussion forum] or join the [IRC channel] and ask there.

### Required Tools

When working with the Kokorowatari code base, we require the following tools:
...
Other than that, your code editor and everything else is up to personal taste, so go ham. If you are having a hard time finding things like [Yosys], [sby], [Yices2], and [Bitwuzla], those, along with a whole lot of other tools are in the [OSS CAD Suite] builds. Instructions in that repository should help you get those set up so you can use them with Kokorowatari.

### Setting Up

1. [Fork] the repository to your GitHub account and clone it locally if you've not done so already previously.
1. Within the local repository, add this repo as a remote: `git remote add upstream https://github.com/shrine-maiden-heavy-industries/kokorowatari`
...
The overall style guidelines for Kokorowatari are not super fixed or well defined at the moment, but the following things are required:
...
***TODO(aki)*** Write details on remote testing setup

To run the Kokorowatari test suite, you can simply invoke `nox -s test`, this will do everything needed to set up Kokorowatari and then invoke it's test suite on the current state of the code base as it is locally.
...
* If the commit touches part of the core Kokorowatari package, try to use the full dotted path to the module. (e.g. `kokorowatari.ddr3`, `kokorowatari.support.lattice`, etc. )
...
```text
kokorowatari.ddr3: Fixed clock delay

Fixed an issue where the DQS clock was delayed one
extra cycle
```
...
Next, the `$function` section can be thought of something like a filesystem path segment that describes where this branch is focused, for instance, `support/lattice` if it's related to the Lattice FPGA support. If the changes are more wide-reaching than a specific sub-section, then this part can be omitted.

Finally, the `$name` section should be a short, but descriptive name for what this branch is doing, it doesn't need to be super detailed, that's what commit messages are for, but something that gives a general idea, like `enumeration-fix`.

So, to put it all together, the following are good examples of how your branch should be named:

* `fix/support/lattice/ecp5`
* `docs/grammar-fixes`

### Submitting a Pull Request

> [!IMPORTANT]
> Kokorowatari uses a [git rebase] based workflow to maintain a linear history and avoid merge commits.

Once your changes are done, tested, and you're happy with them, then now to open a pull request against the Kokorowatari repository.
...
Once your changes are all up to date, make sure that all the tests pass (see the [Testing] section for more info), and then you can then open a [new pull request] against the Kokorowatari repository.
...
And with that, you're done! You've contributed to Kokorowatari!

## Documentation

The documentation for Kokorowatari uses [sphinx] with a few plugins, most notably [MyST], which allows us to use Markdown with some extensions to do the documentation rather than reStructuredText.

When contributing to the Kokorowatari documentation there are 2 key places it can be found, the "free text" documentation is located in the [`docs`] directory in the root of the repository and contains all the main markup for the overall layout and contents of the documentation site, where as the API and most library documentation is done via Python doc comments in the relevant source code itself.
...
Kokorowatari explicitly does not allow any contributions that were in either whole or in part made using a Generative AI system or agent, such as large language models (LLMs), chatbots, or image generation systems. This ban includes, but is not limited to Copilot, BARD/Gemini, ChatGPT, Claude, llama, DeepSeek, Stable Diffusion, DALL-E, Midjourney, or Devin AI.

This policy covers ***ALL*** parts of this project, including but not limited to code, tests, documentation, metadata, artworks, issues, comments, suggestions, feature requests, discussions, pull requests, and any other possible contributions that can be made to Kokorowatari.

If you are found to be using such tools when contributing to Kokorowatari, all of you contributions will be removed if viable, outstanding contributions closed without recourse or review, all future collaboration efforts denied, and you will be banned from contributing to Kokorowatari and related projects.

No exceptions to this policy will be made.

> [!NOTE]
> It is also recommended to avoid any and all AI tools when asking questions about Kokorowatari,
...
[`LICENSE`]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/blob/main/LICENSE
[CC-BY-SA 4.0]: https://creativecommons.org/licenses/by-sa/4.0/
[`LICENSE.docs`]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/blob/main/LICENSE.docs
[git repository]: https://github.com/shrine-maiden-heavy-industries/kokorowatari
[Code Of Conduct]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/blob/main/CODE_OF_CONDUCT.md
...
[issue tracker]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/issues
[report an issue]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/issues
[discussion forum]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/discussions
...
[fork]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/fork
...
[`docs`]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/blob/main/docs
[testing]: #testing
[new pull request]: https://github.com/shrine-maiden-heavy-industries/kokorowatari/compare
